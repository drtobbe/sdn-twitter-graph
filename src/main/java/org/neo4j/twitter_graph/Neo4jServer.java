...
public class Neo4jServer {
    ...
    public static void main(String args[]) {
        GraphDatabaseAPI graphdb = (GraphDatabaseAPI) new GraphDatabaseFactory().newEmbeddedDatabaseBuilder(
                "twitter.db").setConfig(ShellSettings.remote_shell_enabled, "true").newGraphDatabase();

        // let the server endpoint be on a custom port
        ServerConfigurator config = new ServerConfigurator(graphdb);
        config.configuration().setProperty(Configurator.WEBSERVER_PORT_PROPERTY_KEY, 7473);
